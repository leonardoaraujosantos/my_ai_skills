...
- Scripts are Python 3 CLIs with argparse subcommands. Prefer stdlib-only; when a script needs pip packages (Pillow, pypdf, …), document them in the skill's README section and its Dependencies cell in `docs/SKILLS.md`. Smoke-test scripts for real before committing (known reference values for calculators).
- Docs parity is enforced by CI: every skill needs a row in the `docs/SKILLS.md` Skills Overview table (alphabetical), a node in the README "Skills at a Glance" mermaid mindmap, AND a `## <name>` body section in the README (alphabetical).
